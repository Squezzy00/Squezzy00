...
bot.command('broadcast', async (ctx) => {
    ...
    try {
        // Получаем список всех чатов, где есть бот (в реальности нужно хранить их в БД)
        // Здесь просто эмулируем рассылку в текущий чат для примера
        await ctx.reply(`📢 Рассылка от владельца:\n\n${messageText}\n\nDEVELOPER: @SQUEZZY00`);
        
        // В реальной реализации нужно:
        // 1. Хранить список чатов в базе данных
        // 2. Итерироваться по всем чатам и отправлять сообщения
        // 3. Обрабатывать ошибки (например, если бота выгнали из чата)
        
        ctx.reply('✅ Рассылка начата. Сообщение отправлено в тестовом режиме (только в этот чат).\n\nВ реальной реализации нужно хранить список чатов в БД.');
    } catch (e) {
