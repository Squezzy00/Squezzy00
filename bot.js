...
Разработчик: @squezzy00
  `, { reply_markup: { remove_keyboard: true } }); // Убираем основную клавиатуру
});
...
// Обработчик таймеров (русские буквы)
...
bot.command('set', async (ctx) => {
  ...
  try {
    ...
    ctx.replyWithMarkdown(
      ...
      `Разработчик: @squezzy00`,
      Markup.keyboard(buttons).resize().oneTime().selective()
    );
  } catch (err) {
...
bot.command('open', async (ctx) => {
  try {
    ...
    if (userKb.rows.length > 0) {
      return ctx.replyWithMarkdown(
        `✅ *Ваша клавиатура*\n\nРазработчик: @squezzy00`,
        Markup.keyboard(userKb.rows[0].buttons).resize().oneTime().selective()
      );
    }
    ...
    if (ctx.chat.type !== 'private') {
      ...
      if (chatKb.rows.length > 0) {
        return ctx.replyWithMarkdown(
          `👥 *Клавиатура чата*\n\nРазработчик: @squezzy00`,
          Markup.keyboard(chatKb.rows[0].buttons).resize().oneTime().selective()
        );
      }
    }
    ...
    ctx.reply('❌ Ошибка загрузки', { reply_markup: { remove        id SERIAL PRIMARY KEY,
        user_id BIGINT NOT NULL,
        username TEXT,
        text TEXT NOT NULL,
        end_time BIGINT NOT NULL,
